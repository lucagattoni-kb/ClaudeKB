...
PY
( cd "$FX" && git add -A && git -c user.email=ci@ci -c user.name=ci commit -qm "add page" \
...
(
  ...
  git -c user.email=a@a -c user.name=a commit -qam "A"
  git checkout -q main
  printf '\n## 20260712 10:01 ingest | Session B\nB body.\n' >> docs/log.md
  git -c user.email=b@b -c user.name=b commit -qam "B"
